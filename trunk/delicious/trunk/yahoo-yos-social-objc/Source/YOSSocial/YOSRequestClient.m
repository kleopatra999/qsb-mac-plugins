...
- (YOSResponseData *)sendSynchronousRequest
{
	...
	//[rspError autorelease];	this was causing a seg fault
...
- (NSMutableURLRequest *)buildUrlRequest
{
	...
	if([self.HTTPMethod isEqualToString:@"POST"])
	{
		...
		NSString *requestAbsoluteURLString = [queryParameters count] != 0
     ? [NSString stringWithFormat:@"%@?%@", [self.requestUrl absoluteString], [queryParameters QueryString]]
	...
	if(oauthRequest) {
      ...
      [urlRequest setValue:[oauthRequest buildAuthorizationHeaderValue] forHTTPHeaderField:@"Authorization"];
		}
